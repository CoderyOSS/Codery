...
FIRST_KEY=$(find "$SSH_DIR" -maxdepth 1 -type f \
  ...
  | sort | head -1)
if [ -z "$FIRST_KEY" ]; then
  echo "[apps] No private SSH keys found, skipping"
  exit 0
fi
...
if ssh-add "${FIRST_KEY}" 2>/dev/null; then
  echo "[apps] SSH agent started, loaded key: $(basename "${FIRST_KEY}")"
else
  echo "[apps] Warning: Could not load key: $(basename "${FIRST_KEY}")"
fi
