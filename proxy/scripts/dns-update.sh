...
# Keep TAILSCALE_IP current in .env so the orchestrator's caddy reload picks it up.
if [ -f /opt/codery/.env ]; then
  sed -i '/^TAILSCALE_IP=/d' /opt/codery/.env
  echo "TAILSCALE_IP=$TS_IP" >> /opt/codery/.env
  echo "[dns-update] Updated TAILSCALE_IP in /opt/codery/.env"
fi
